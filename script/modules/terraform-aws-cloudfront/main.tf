...
resource "aws_cloudfront_distribution" "cf_distribution" {
  ...
  dynamic "origin" {
    for_each = [for o in var.custom_origin_configs : {
      ...
      custom_header            = o.custom_headers
    ...
    content {
      ...
      origin_path = origin.value.origin_path
      custom_header {
        name = origin.value.custom_header.name
        value = origin.value.custom_header.value
      }
  ...
  dynamic "ordered_cache_behavior" {
    ...
    for_each = [for s in var.ordered_cache_behavior_variables : {
      ...
      headers                = s.headers
      forward                = s.forward   # Cookie forward
      whitelisted_names      = s.whitelisted_names
      min_ttl                = s.min_ttl
  ...
  dynamic "viewer_certificate" {
    for_each = "${var.viewer_certificate_type == "acm" ? local.acm_certificate_arn_configs[local.enable_certificate] : []}"
    content {
  ...
  dynamic "viewer_certificate" {
    for_each = "${var.viewer_certificate_type =="cloudfront" ? list("dummy") : []}"
    content {
  ...
  restrictions {
    geo_restriction {
      # How to restrict distribution of your content by country: none, whitelist, or blacklist.
